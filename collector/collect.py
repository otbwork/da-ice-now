# -*- coding: utf-8 -*-
"""
Da-iCE 最新情報アグリゲーター - データ収集スクリプト

完全無料の情報源（APIキー不要）だけを使って Da-iCE の最新情報を集約し、
...
NEWS_QUERIES = [
    '"Da-iCE"',
    '"Da-iCE" avex OR ライブ OR 新曲 OR リリース',
]

# 公式 YouTube ハンドル / チャンネル（channel_id は実行時に解決する）
# 公式チャンネル: Da-iCE OFFICIAL CHANNEL（ID: UC7JQKVJMCIuVMcrFXSozMCw）
YOUTUBE_HANDLES = ["c/DaiCECH"]
...
UA = {"User-Agent": "Mozilla/5.0 (compatible; DaiceFeedBot/1.0; +https://github.com)"}
...
def fetch_youtube() -> list:
    ...
    for handle in YOUTUBE_HANDLES:
        ...
        try:
            ...
            continue
        # YOUTUBE_HANDLES は公式チャンネルのみを登録しているため、すべて official 扱い
        for e in entries:
            title = clean_text(e.get("title", ""))
            items.append({
                "type": "youtube",
